...
def test_runtime_b11_rejects_generator_input_and_origin_mutation(tmp_path: Path) -> None:
    checkout = _git_repo(tmp_path)
    original = (checkout / "contract.json").read_bytes()
    ...
    with pytest.raises(ReleaseIdentityError):
        run_b11_1(
            [resolve_component("NEUTRAL-RUNTIME", checkout)],
            [{"generator_id": "GEN", "component_id": "NEUTRAL-RUNTIME", "checkout": str(checkout), "entrypoint_path": "generator.py"}],
            contract_bytes=b"{}", schema_manifest_bytes=b"{}", output_root=tmp_path,
        )
    assert (checkout / "contract.json").read_bytes() == original
