        ...
        for d in sorted((dict(x) for x in generator_definitions),key=lambda x:x.get("generator_id","")):
            ...
            for n in (1,2):
                ...
                if component_manifest_bytes is not None:
                    component_path=inp/"component-manifest.json";component_path.write_bytes(component_manifest_bytes);input_digests[component_path]=hashlib.sha256(component_manifest_bytes).hexdigest()
                checkout_snapshot=_filesystem_snapshot(checkout);exe=str(d.get("interpreter_path") or d.get("executable") or "python3");argv=[exe,str(ep_run),"--contract",str(cp),"--schema-manifest",str(sp)]
                if component_path is not None:argv.extend(["--component-manifest",str(component_path)])
                argv.extend(["--output-root",str(stage)]);before=_dirty(checkout);res=subprocess.run(_sandbox_command(argv,staging=stage),cwd=str(checkout),capture_output=True,check=False,timeout=300,env={"PATH":os.environ.get("PATH","") ,"PYTHONDONTWRITEBYTECODE":"1","PYTHONNOUSERSITE":"1"});after=_dirty(checkout)
                ...
                for path,digest in input_digests.items():
                    if not path.is_file() or hashlib.sha256(path.read_bytes()).hexdigest()!=digest:raise ReleaseIdentityError("B11.1 generator changed its contract, schema, or component-manifest input")
                ...
                stable_argv=["<interpreter>","<component-checkout>/"+str(d["entrypoint_path"]),"--contract","<contract-input>","--schema-manifest","<schema-manifest-input>"]
                if component_path is not None:stable_argv.extend(["--component-manifest","<component-manifest-input>"])
                stable_argv.extend(["--output-root","<staging-output-root>"]);invs.append(inv);receipts.append({"schema_version":1,"artifact_kind":"generator-run-receipt","generator_id":gid,"run_ordinal":n,"argv":stable_argv,"argv_sha256":framed_hash("banodoco.generator-run-argv.v1",stable_argv),"output_rows":inv,"exit_code":0})
            ...
            contract_digest=hashlib.sha256(contract_bytes).hexdigest();schema_digest=hashlib.sha256(schema_manifest_bytes).hexdigest();observation_input_digests=[contract_digest,schema_digest];observation_bindings=[{"input_id":"CONTRACT-ID","sha256":contract_digest},{"input_id":"EXECUTION-SCHEMAS-MANIFEST","sha256":schema_digest}]
            if component_manifest_bytes is not None:
                component_digest=hashlib.sha256(component_manifest_bytes).hexdigest();observation_input_digests.append(component_digest);observation_bindings.append({"input_id":"COMPONENT-MANIFEST","sha256":component_digest})
            observation_bindings.append({"input_id":"GENERATOR-DEFINITION","sha256":dd})
            obs={"schema_version":1,"row_kind":"OBSERVATION","generator_id":gid,"component_id":cid,"entrypoint_component_id":cid,"entrypoint_path":str(d["entrypoint_path"]),"entrypoint_sha256":hashlib.sha256(ep.read_bytes()).hexdigest(),"interpreter_tool_id":d.get("interpreter_tool_id","TOOL-PYTHON"),"argv_formula_id":"GENERATOR-ARGV-V1","sandbox_policy_id":"GENERATOR-READONLY-STAGING-V1","generator_definition_sha256":dd,"input_schema_ids":list(d.get("input_schema_ids",[])),"input_digests":observation_input_digests,"declared_output_roots":list(d.get("declared_output_roots",["."])),"tool_ids":list(d.get("tool_ids",["TOOL-GIT","TOOL-PYTHON"])),"output_paths":[x["path"] for x in invs[0]],"output_digests":[x["sha256"] for x in invs[0]],"tool_rows":list(d.get("tool_rows",[])),"run_ordinal":NONE,"argv_carrier":NONE,"argv_sha256":NONE,"clean_checkout_id":NONE,"changed_paths":[],"undeclared_changed_paths":[],"started_at":NONE,"finished_at":NONE,"exit_code":NONE,"stop_class":NONE,"first_run_receipt_sha256":hashlib.sha256(canonical_bytes(receipts[0])).hexdigest(),"second_run_receipt_sha256":hashlib.sha256(canonical_bytes(receipts[1])).hexdigest(),"run_receipt_evidence_rows":rr,"provenance_input_bindings":observation_bindings,"producer_id":"PROD-CMD-PACKET:B11.1"}
...
def create_pre_live_identity(components:Mapping[str,str|os.PathLike[str]],*,metadata:Mapping[str,Any]|None=None,output:Any=None,seed_outputs:Mapping[str,Any]|None=None,generator_definitions:Sequence[Mapping[str,Any]]|None=None,contract_bytes:bytes|None=None,schema_manifest_bytes:bytes|None=None,component_manifest_bytes:bytes|None=None,registry_rows:Sequence[Mapping[str,Any]]|None=None)->dict[str,Any]:
...
def verify_receipt(r:Mapping[str,Any])->str:
    ...
    if r.get("kind")=="pre-live-root":
        ...
        if r.get("pre_live_root")!=core.get("pre_live_evidence_root"):raise ReleaseIdentityError("candidate pre-live root does not match candidate core evidence root")
        m=r.get("pre_live_manifest")
        ...
        reconstructed=framed_hash("banodoco.pre-live-evidence-root.v1",{"component_rows":[prior_rows[c] for c in sorted(prior_rows)],"evidence_rows":r.get("pre_live_evidence_rows"),"manifest_sha256":m["manifest_sha256"]})
        if reconstructed!=r.get("pre_live_root"):raise ReleaseIdentityError("candidate pre-live root is not a verified PRELIVE root")
